"""
PCA computation module for dimensionality reduction.
Implements PCA algorithms using both covariance matrix and SVD methods.
"""
...
def run_pca(
    data_matrix: np.ndarray, k: int = None, variance_threshold: float = None
) -> tuple:
    """
    ...
    if variance_threshold is not None and (
        variance_threshold <= 0 or variance_threshold > 1
    ):
