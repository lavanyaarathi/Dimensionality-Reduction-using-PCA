...
from modules.upload_handler import FileMetadata, FileUploadHandler
...
def generate_variance_plot(eigenvalues, k_used, session_dir, base_filename):
    ...
    try:
        ...
        ax.bar(x, variance_ratio, alpha=0.6,
        ...
        ax2.axhline(
            y=0.95, color="red", linestyle="--", linewidth=1, label="95% cutoff"
        )
    ...
    try:
        ...
        if pca_output.shape[0] > max_points:
            indices = np.random.choice(
                pca_output.shape[0], max_points, replace=False)
        ...
        if k_used >= 3:
            ...
            scatter = ax.scatter(
                pca_sample[:, 0],
                pca_sample[:, 1] if k_used >= 2 else np.zeros_like(
                    pca_sample[:, 0]),
                c=pca_sample[:, 0],
                cmap="viridis",
                alpha=0.6,
...
def verify_token(current_user):
    return jsonify({"message": "Token is valid!", "username": current_user}), 200
...
def create_session(current_user):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": f"Failed to create session: {str(e)}"}),
            500,
        )
...
def upload_file(current_user):
    ...
    if not session_manager.session_exists(session_id):
        return (
            jsonify({"success": False, "error": "Invalid or expired session_id"}),
            400,
        )
...
def run_pca_endpoint(current_user):
    ...
    if not session_id or not filename:
        return (
            jsonify(
                {"success": False, "error": "session_id and filename are required"}
            ),
            400,
        )
    ...
    if not filepath:
        return (
            jsonify({"success": False, "error": "File not found for given session"}),
            404,
        )
    ...
    try:
        if file_type == "tabular":
            ...
            for col in df.columns:
                if df[col].dtype == "object":
                    try:
                        ...
                        if (
                            date_col.notna().sum() > len(df) * 0.5
                        ):  # If >50% are valid dates
            ...
            if numeric_df.shape[1] < 2:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": f'Insufficient numeric columns for PCA. Found {
                                numeric_df.shape[1]} numeric column(s). Need at least 2. Available columns: {
                                ", ".join(available_cols)}',
                        }),
                    400,
                )
            ...
            if numeric_df.empty:
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "No numeric data found after processing",
                        }
                    ),
                    400,
                )
            ...
            if k is None and variance_threshold is None:
                return (
                    jsonify(
                        {"success": False, "error": "Provide k or variance_threshold"}
                    ),
                    400,
                )
            if k is not None:
                try:
                    ...
                    return (
                        jsonify(
                            {"success": False, "error": "k must be an integer"}),
                        400,
                    )
                if k <= 0 or k > n_features:
                    return (
                        jsonify(
                            {
                                "success": False,
                                "error": f"k must be between 1 and {n_features}",
                            }
                        ),
                        400,
                    )
            ...
            pca_output, eigenpairs = run_pca_svd(
                data_centered, k=k, variance_threshold=variance_threshold
            )
            ...
            transformed_df = pd.DataFrame(
                pca_output, columns=pc_cols, index=numeric_df.index
            )
            ...
            # Generate plots
            variance_plot_name = generate_variance_plot(
                eigenpairs["eigenvalues"], k_used, session_dir, base_filename
            )
            scatter_plot_name = generate_scatter_plot(
                pca_output, k_used, session_dir, base_filename
            )
            ...
            if session:
                sample_idx = np.linspace(
                    0,
                    pca_output.shape[0] - 1,
                    num=min(200, pca_output.shape[0]),
                    dtype=int,
                )
                session.set_pca_result(
                    filename,
                    {
                        "explained_variance_ratio": (
                            eigenpairs["eigenvalues"][:k_used]
                            / np.sum(eigenpairs["eigenvalues"])
                        ).tolist(),
            ...
            if variance_plot_name:
                response_data["variance_plot_endpoint"] = (
                    f"/api/pca/download?session_id={session_id}&outfile={variance_plot_name}")
            if scatter_plot_name:
                response_data["scatter_plot_endpoint"] = (
                    f"/api/pca/download?session_id={session_id}&outfile={scatter_plot_name}")
                ...
                if k <= 0 or k > c:
                    return (
                        jsonify(
                            {
                                "success": False,
                                "error": f"k must be between 1 and {c} for images",
                            }
                        ),
                        400,
                    )
            ...
            reconstructed_centered = np.dot(pca_output, eigenvectors_k.T)
            reconstructed = reconstructed_centered + mean_vec
            recon_img = np.clip(reconstructed.reshape(
                h, w, c), 0, 255).astype(np.uint8)
            ...
            return (
                jsonify(
                    {"success": False, "error": "Unsupported file type for PCA"}),
                400,
            )
...
def download_reconstructed(current_user):
    ...
    if not session_id or not outfile:
        return (
            jsonify(
                {"success": False, "error": "session_id and outfile are required"}),
            400,
        )
    ...
    if not os.path.exists(full_path):
        ...
        return (
            jsonify(
                {"success": False, "error": f"Output file not found: {outfile}"}),
            404,
        )
    ...
    return send_file(
        full_path, mimetype=mime, as_attachment=True, download_name=outfile
    )
...
def preview_image():
    ...
    if not os.path.exists(full_path):
        ...
        print(
            f"Session dir contents: {
                os.listdir(session_dir) if os.path.exists(session_dir) else 'Dir not found'}")
