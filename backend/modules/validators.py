"""
File validation module for PCA project.
Handles validation of uploaded files (tabular and image data).
"""
...
class FileValidator:
    ...
    def allowed_file(filename, file_type="all"):
        ...
        if file_type == "all":
            all_extensions = (
                Config.ALLOWED_EXTENSIONS["tabular"]
...
class TabularValidator:
    ...
    def validate_excel(filepath):
        ...
        try:
            ...
            return (
                True,
                f"Valid Excel with {df.shape[0]} rows and {df.shape[1]} columns",
            )
...
class ImageValidator:
    ...
    def validate_image(filepath):
        ...
        try:
            ...
            # Validate dimensions
            if (
                width < Config.MIN_IMAGE_DIMENSION
                or height < Config.MIN_IMAGE_DIMENSION
            ):
                return False, (f"Image dimensions too small " f"(minimum {
                    Config.MIN_IMAGE_DIMENSION}x{
                    Config.MIN_IMAGE_DIMENSION})")

            if (
                width > Config.MAX_IMAGE_DIMENSION
                or height > Config.MAX_IMAGE_DIMENSION
            ):
                return False, (f"Image dimensions too large " f"(maximum {
                    Config.MAX_IMAGE_DIMENSION}x{
                    Config.MAX_IMAGE_DIMENSION})")

            # Check if image is RGB or can be converted
            if img.mode not in ["RGB", "L", "RGBA"]:
                return (
                    False, f"Unsupported image mode: {
                        img.mode}. Expected RGB, RGBA, or grayscale", )
